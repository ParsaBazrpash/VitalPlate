...
interface FoodRecommendations {
  [key: string]: FoodRecommendation | Greeting;
}
...
const HealthFoodRecommender: React.FC = () => {
  ...
  const symptomKeywords = {
    ...
    'goodbye': ['goodbye', 'bye', 'see you', 'farewell', 'later'],
  ...
  const greetings = {
    'hello': {
      text: [
        "Hello! I'm here to help you find the right foods for your needs.",
        "Whether you're looking for recipes to soothe a sore throat or boost your energy, I've got you covered.",
        "Just let me know what you're looking for, and I'll provide you with some personalized recommendations."
      ]
    },
    'goodbye': {
      text: [
        "Goodbye! I hope you found the information helpful and that you enjoy the recipes.",
        "If you have any other questions or need more recommendations in the future, feel free to ask.",
  ...
  useEffect(() => {
    const loadFoodRecommendations = async () => {
      try {
        ...
        setFoodRecommendations({ ...data, ...greetings });
      } catch (err) {
  ...
  const generateResponse = (detectedSymptoms: string[]): MessageProps => {
    try {
      ...
      if (detectedSymptoms.includes('hello')) {
        const greeting = foodRecommendations['hello'] as Greeting;
        return {
          type: 'bot',
          message: (
            <div className="space-y-2">
              {greeting.text.map((line, index) => (
                <p key={index}>{line}</p>
              ))}
            </div>
          )
      ...
      if (detectedSymptoms.includes('goodbye')) {
        const farewell = foodRecommendations['goodbye'] as Greeting;
        return {
          type: 'bot',
          message: (
            <div className="space-y-2">
              {farewell.text.map((line, index) => (
                <p key={index}>{line}</p>
              ))}
            </div>
          )
      ...
      // Handle regular food recommendations
      const recommendations = detectedSymptoms
        .filter(symptom => foodRecommendations[symptom] && 'foods' in foodRecommendations[symptom])
        .map(symptom => ({
          symptom,
          ...(foodRecommendations[symptom] as FoodRecommendation)
        }));
